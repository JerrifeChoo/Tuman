...
<h4>三、主系统继承ISystem支持BurstCompile，回调系统继承SystemBase支持托管类型。</h4>
<h4>四、无SubScene子场景，动态创建EntityPrefab用于计时器实例化。</h4>
<h4>五、支持Mono，支持Time.timeScale缩放，支持暂停/恢复时长补偿。</h4>
<h4>六、基于Time.realtimeSinceStartup计算，减少多次累加导致的精度丢失，比Time.deltaTime更精准。</h4>
<h2>TimerSystem</h2>计时器主系统，包含三种执行方式主线程、单线程、多线程并发，根据工作线程数、当前实体数、Chunk数实时调整。
<h2>HandleSystem</h2>包含计时器回调、销毁回调涉及托管数据内容的回调处理系统。
...
<h4>$${\color{blue}onDestroy,int \space repeatCount=1, bool \space ignoreScale = false, bool \space ignoreGap = false)}$$</h4>
