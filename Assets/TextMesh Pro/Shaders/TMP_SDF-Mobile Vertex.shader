...
Shader "TextMeshPro/Mobile/Distance Field Vertex" {
...
SubShader {
	...
	Pass {
		...
		#pragma shader_feature __ OUTLINE_ON OUTLINE_OUTER
		...
		fixed4 PixShader(pixel_t input) : SV_Target
		{
			...
			#ifdef OUTLINE_OUTER
			half w = input.param.y - input.param.w;
			c = lerp(input.outlineColor, input.faceColor, saturate(d - input.param.z - w));
			c *= saturate(d - input.param.y - w);
			#endif
