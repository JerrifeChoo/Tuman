...
public static class ExampleConfig
{
    ...
    public static List<Type> CSharpCallLua
    {
        get
        {
            ...
            return delegate_types.Where(t => t.BaseType == typeof(MulticastDelegate) && !hasGenericParameter(t) && !delegateHasEditorRef(t) && !(isExcludeDelegate(t))).Distinct().ToList();
        }
    }

    public static List<List<string>> BlackDelegateList = new List<List<string>>()
    {
        new List<string>(){"UnityEngine.Application", "MemoryUsageChangedCallback"},
    };

    static bool isExcludeDelegate(Type delegateType)
    {
        foreach (var exclude in BlackDelegateList)
        {
            if (delegateType.DeclaringType!=null && delegateType.DeclaringType.ToString() == exclude[0] && delegateType.Name == exclude[1])
            {
                return true;
            }
        }
